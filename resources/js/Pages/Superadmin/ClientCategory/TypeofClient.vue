...
const label = computed(() => editingClient.value ? 'Update Client' : 'Add Client');
...
const editClient = (client) => {
    ...
    form.typeofclient = client.typeofclient;
...
const submit = () => {
    ...
    const routeName = isEdit
        ? route('client-type.update', editingClient.value.id)
        : route('client-type.store');
        ...
        <DataTable
            ...
            class="p-datatable-sm text-sm min-w-[600px]"
             removableSort
                ...
                scrollHeight="flex"
        >
            <Column field="typeofclient" header="Type of Client">
                <template #body="slotProps">
                    <span class="text-sm text-gray-800">
                        {{ slotProps.data.typeofclient }}
                    </span>
                </template>
            </Column>
